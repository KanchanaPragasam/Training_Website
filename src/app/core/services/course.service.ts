...
export class CourseService {
  private paths = {
    all: '../../../assets/data/courses.json',
    featured: '../../../assets/data/trending_courses.json',
    internship: '../../../assets/data/internships.json'
  };
  ...
  // ---------------- Internship ----------------
  getInternshipData(xmlPath: string = this.paths.internship): Observable<Program[]> {
    return this.http.get<Program[]>(xmlPath);
  }

  // ---------------- Courses ----------------
  getCourses(source: CourseSource = 'all'): Observable<Course[]> {
    if (this.coursesCache.has(source)) {
      return of(this.coursesCache.get(source)!);
    }
    ...
    return this.http.get<Course[]>(path).pipe(
      map(courses => {
        this.coursesCache.set(source, courses);
        this.setAllSlugs(courses.map(c => c.slug));
        return courses;
      })
    );
  }
  ...
  getFullCourseById(slug: string, source: CourseSource = 'all'): Observable<Course | undefined> {
    ...
    const cachedCourses = this.coursesCache.get(source);
    if (cachedCourses) {
      const course = cachedCourses.find(c => c.slug === slug);
      if (course) {
        this.fullCoursesCache.set(cacheKey, course);
        return of(course);
      }
    ...
    return this.http.get<Course[]>(path).pipe(
      map(courses => {
        ...
        if (course) this.fullCoursesCache.set(cacheKey, course);
        return course;
      })
    );
  }
  ...
  getCourseNames(source: CourseSource = 'all'): Observable<string[]> {
    if (this.coursesCache.has(source)) {
      ...
      return of(this.coursesCache.get(source)!.map(c => c.courseTitle).filter(name => !!name));
    }
    ...
    return this.http.get<Course[]>(path).pipe(
      map(courses => {
        this.coursesCache.set(source, courses);
        return courses.map(c => c.courseTitle).filter(name => !!name);
      })
    );
  }

  // ---------------- Helper ----------------
  private parseCourse(node: any): Course {
    // Optional if you want to adapt JSON nodes further
    return {
      ...node
    };
  }
